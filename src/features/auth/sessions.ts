...
const deleteSessionAndSetCookie = async (session_id: string) => {
    await db.delete(sessionsTableSchema).where(eq(sessionsTableSchema.sessionId,session_id));
    const cookieStore = await cookies();
...
export const getCurrentUser = cache(async () => {
    try{
        ...
        const session_id = crypto.createHash('sha-256').update(rawHexId).digest("hex");
        ...
        if(isExpired){
            ...
            await deleteSessionAndSetCookie(session_id);
